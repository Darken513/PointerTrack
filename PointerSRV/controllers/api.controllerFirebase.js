...
exports.getAllRestaurants = async (req, res) => {
  try {
    ...
    const restaurants = snapshot.docs.filter(doc => doc.data().enabled).map((doc) => { return { id: doc.id, ...doc.data() } })
...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    const users = snapshot.docs.filter(doc => doc.data().enabled).map((doc) => { return { id: doc.id, name: doc.data().name } });
    res.json({ users });
  } catch (error) {
    console.error('Error getting documents:', error);
    res.json({ title: "Error", body: "No Data available." });
...
exports.createUser = async (req, res) => {
  try {
    const data = { code: req.body.code, name: req.body.username, enabled: true, pointed: false, created_at: new Date() }
...
exports.deleteRestaurantById = async (req, res) => {
  try {
    ...
    await docRef.delete();
    console.log('Restaurant with ID:', req.params.restaurantId, 'deleted successfully.');
    res.json({ done: true })
  } catch (error) {
  ...
  try {
    ...
    if (!snapshot.empty) {
      ...
      for (let i = 0; i < snapshot.docs.length; i++) {
        ...
        toret.push({ user, action: doc.data().action, restaurant, time: doc.data().created_at });
      }
      ...
      toret = toret.map((data) => { return { user: data.user.name, action: data.action, restaurant: data.restaurant.name, time: data.time.toDate() } })
      toret.sort((a, b) => {
        // Compare by name
      ...
      let afterCalc2 = calculateTimeSpentForEach_User_RestaurantDeveloped(parsedData2);
      ...
      try {
        ...
        if (toDownload) {
          ...
          sendMail(req.body);
          res.status(201).json({ title: "success", body: "Email sent successfully" });
        }
      } catch (error) {
        console.error('Error writing CSV file:', error);
        res.status(201).json({ title: "error", body: "Error writing CSV file" });
      }
    }
  } catch (error) {
    console.error('Error getting documents:', error);
  ...
  Object.keys(object).forEach(userName => {
    ...
    Object.keys(userObj).forEach(restaurantName => {
      ...
      restaurantObj.detailed.forEach(details => {
        toPush = { user: userName, restaurant: restaurantName, started_at: details.start, ended_at: details.end, time: details.total }
        toret.push(toPush);
      })
    })
  })
...
function sendMail(fromToDetails) {
  ...
  const mailOptions = {
    ...
    attachments: [
      {
        filename: 'Rapport_de_+' + fromToDetails.startDate + "_" + fromToDetails.endDate + '_total.csv',
        path: 'output1.csv',
      },
      {
        filename: 'Rapport_de_+' + fromToDetails.startDate + "_" + fromToDetails.endDate + '_détaillé.csv',
        path: 'output2.csv',
      },
    ],
  };
